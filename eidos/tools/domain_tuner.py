...
def load_engine_module() -> Any:
    ...
    # Prefer the refactored engine when it exposes the streaming entrypoint;
    # fall back to the monolith in checkouts where the package copy lags it.
    ...
    checked: List[str] = []
    for engine_path in (refactored_path, monolith_path):
        if not engine_path.exists():
            continue
        spec = importlib.util.spec_from_file_location("eidos_engine", engine_path)
        if spec is None or spec.loader is None:
            checked.append(f"{engine_path}: import spec unavailable")
            continue
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        if hasattr(module, "run_stream_once"):
            return module
        checked.append(f"{engine_path}: missing run_stream_once")
    raise RuntimeError("No runnable Eidos engine found; checked " + "; ".join(checked))
