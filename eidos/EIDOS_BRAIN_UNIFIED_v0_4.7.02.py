...
ARTIFACT_ROOT_PREFERRED = os.environ.get("EIDOS_ARTIFACT_ROOT", r"E:\agent data")
...
EIDOS_BRAIN_CONFIG = {
    ...
    # =========================================================================
    # LEAP IV (Preview) — TRACE SEAL + DIAGONAL CUTTER (Surgical Patch)
    # =========================================================================
    ...
    "demo_window": 400,
    "demo_write_csv": True,
    "demo_plot_matplotlib": False,
    "demo_out_csv": "demo_trace_seal.csv",

    # =========================================================================
    # LEAP IV / V — FORECAST, INCIDENTS, PROCEDURAL MEMORY
    # =========================================================================
    "mode": "detect",           # detect | forecast | hybrid
    "domain": "generic",        # generic | cyber | web | dataset | flight | healthcare

    # Incident cards
    "incident_cards_enabled": True,
    "incident_emit_on": ["AMBER", "RED"],
    "incident_min_gap_steps": 25,

    # Procedural memory
    "procedural_enabled": True,
    "procedural_policy": "recommend",  # recommend | auto
    "procedural_actions": "default",
    "procedural_min_similarity": 0.35,
    "procedural_reward_horizon": 150,

    # Forecast mode
    "forecast_enabled": False,
    "forecast_mode": "risk",           # risk | trajectory
    "forecast_horizon_steps": [10, 30, 100],
    "forecast_emit_every": 10,
    "forecast_traj_window": 30,
    "forecast_min_conf": 0.35,
...
def validate_config(conf: Dict[str, Any]) -> None:
    ...
    # Value checks
    if conf["steps"] <= 0: raise ValueError("steps must be positive")
    if conf["reservoir"] <= 0: raise ValueError("reservoir must be positive")
...
# --- NEW MODULES ---
try:
    import eidos_domain_adapters
    import eidos_incident_cards
    import eidos_procedural_memory
    import eidos_forecast
except ImportError:
    # If running in Colab/Notebook where these might be file-embedded or need path fix
    ...
    try:
        ...
        print("!! Warning: New EIDOS modules not found. Some features will be disabled.")
...
# =============================================================================
# LEAP IV (Preview) — TRACE SEAL + DIAGONAL CUTTER
# =============================================================================

class TraceSealProjector:
    """
    Positivity-preserving anomaly score with subspace removal (Diagonal-Cutter).

    - Maintains EMA covariance of residuals.
    - Periodically computes P = I - U_k @ U_k.T (removes top-k predictable modes).
    - score(r) = sqrt(r^T P r) => guaranteed >= 0.
    """
    def __init__(self, features: int, device: torch.device, dtype: torch.dtype, cfg: Dict[str, Any]):
        self.features = int(features)
        self.device = device
        self.dtype = dtype

        self.enabled = bool(cfg.get("trace_seal_enabled", False))
        self.rank = int(cfg.get("trace_seal_rank", 4))
        self.decay = float(cfg.get("trace_seal_decay", 0.995))
        self.eig_floor = float(cfg.get("trace_seal_eig_floor", 1e-6))
        self.recalc_every = int(cfg.get("trace_seal_recalc_every", 250))

        # EMA Covariance C (symmetric)
        self.C = torch.eye(features, device=device, dtype=dtype)

        # Projector P (starts as identity -> "no cut")
        self.P = torch.eye(features, device=device, dtype=dtype)

        self.updates = 0
        self.last_recalc = 0

        # Diagnostics
        self.top_eigs: List[float] = []
        self.explained_ratio = 0.0
        self.log_cond = 0.0 # log10(cond)

    def update(self, r: torch.Tensor) -> None:
        """Update covariance with residual vector r."""
        if not self.enabled:
            return

        # r is (features,) or (1, features)
        r = r.reshape(-1, 1).to(self.dtype)

        # C_new = decay * C_old + (1 - decay) * (r @ r.T)
        # We enforce symmetry
        rrT = r @ r.T
        self.C.mul_(self.decay).add_(rrT, alpha=(1.0 - self.decay))

        # Enforce symmetry (safe for partial updates)
        self.C = 0.5 * (self.C + self.C.T)

        self.updates += 1

        # Recompute P?
        if (self.updates - self.last_recalc) >= self.recalc_every:
            self._recalc_projector()
            self.last_recalc = self.updates

    def _recalc_projector(self) -> None:
        try:
            # Eigen decomposition of symmetric matrix
            # eigh returns eigenvalues in ascending order
            L, V = torch.linalg.eigh(self.C)

            # Filter negative eigvals (numerical noise)
            L = torch.clamp(L, min=self.eig_floor)

            total_energy = L.sum().item()

            # Top-k indices (last k elements)
            k = min(self.rank, self.features - 1)

            # Check condition number
            cond = L[-1] / (L[0] + 1e-12)
            self.log_cond = float(torch.log10(cond).item())

            if k > 0:
                # U_k = vectors of top k modes
                # V is (D, D), columns are eigenvectors
                U_k = V[:, -k:] # Shape (D, k)

                # Energy explained by removed subspace
                removed_energy = L[-k:].sum().item()
                self.explained_ratio = removed_energy / (total_energy + 1e-12)

                # P = I - U_k @ U_k.T
                I = torch.eye(self.features, device=self.device, dtype=self.dtype)
                self.P = I - U_k @ U_k.T

                self.top_eigs = L[-5:].detach().cpu().numpy().tolist()[::-1] # Store top 5 desc
            else:
                self.P = torch.eye(self.features, device=self.device, dtype=self.dtype)
                self.explained_ratio = 0.0
                self.top_eigs = []

        except Exception as e:
            print(f"!!! TraceSeal Recalc Failed: {e}")
            # Fallback to Identity
            self.P = torch.eye(self.features, device=self.device, dtype=self.dtype)

    def score(self, r: torch.Tensor) -> float:
        """Compute sqrt(r^T P r)."""
        if not self.enabled:
            return 0.0
        ...
        # Clamp non-negativity (numerical noise could make it -1e-10)
    ...
    def diagnostics(self) -> Dict[str, Any]:
        return {
            ...
            "top_eigs": self.top_eigs,
            "explained": self.explained_ratio,
...
class SessionRecorder:
    ...
    def record_step(
        ...
        *,
        ...
        trace_seal_err: Optional[float] = None,
        trace_seal_expl: Optional[float] = None,
        trace_seal_logcond: Optional[float] = None,
    ) -> None:
        self.step_rows.append(
            {
                ...
                "ts_err": None if trace_seal_err is None else float(trace_seal_err),
                "ts_expl": None if trace_seal_expl is None else float(trace_seal_expl),
                "ts_cond": None if trace_seal_logcond is None else float(trace_seal_logcond),
            }
        )
...
def run_sentinel_stream(
    *,
    ...
    validate_config(EIDOS_BRAIN_CONFIG)
    ...
    trace_seal = TraceSealProjector(
        features=features,
        device=device,
        dtype=DTYPE,
        cfg=EIDOS_BRAIN_CONFIG
    )
    ...
    # --- LEAP IV/V MODULES INGESTION ---
    domain_name = EIDOS_BRAIN_CONFIG.get("domain", "generic")
    
    # Use the imported module
    try:
        domain_adapter = eidos_domain_adapters.get_domain_adapter(domain_name)
    except NameError:
        # Fallback if import failed
        class MockAdapter: 
            def extract(self, x): return {"x": x}
        domain_adapter = MockAdapter()

    try:
        incident_emitter = eidos_incident_cards.IncidentCardEmitter(
            enabled=EIDOS_BRAIN_CONFIG.get("incident_cards_enabled", True),
            min_gap_steps=EIDOS_BRAIN_CONFIG.get("incident_min_gap_steps", 25)
        )
        episode_index = eidos_incident_cards.EpisodeIndex(maxlen=5000)
        procedural_memory = eidos_procedural_memory.ProceduralMemory(
            domain=domain_name,
            policy=EIDOS_BRAIN_CONFIG.get("procedural_policy", "recommend"),
            min_similarity=EIDOS_BRAIN_CONFIG.get("procedural_min_similarity", 0.35),
            enabled=EIDOS_BRAIN_CONFIG.get("procedural_enabled", True)
        )
        forecast_engine = eidos_forecast.ForecastEngine(
            window=EIDOS_BRAIN_CONFIG.get("forecast_traj_window", 30),
            horizons=EIDOS_BRAIN_CONFIG.get("forecast_horizon_steps", [10, 30, 100]),
            temp=EIDOS_BRAIN_CONFIG.get("forecast_similarity_temperature", 6.0),
            enabled=EIDOS_BRAIN_CONFIG.get("forecast_enabled", False)
        )
    except NameError:
        print("!! LEAP IV/V Modules not loaded correctly. Skipping instantiation.")
        incident_emitter = None
        episode_index = None
    ...
    for i, (raw_event, meta_in) in enumerate(gen):
        # --- ADAPTER: Normalize Input ---
        # Delegate feature extraction to Adapter
        adapted = domain_adapter.extract(raw_event)
        frame_np = adapted["x"]
        
        # Merge metadata
        meta = meta_in if isinstance(meta_in, dict) else {}
        if isinstance(meta, dict):
            # Prioritize adapter entities but keep existing meta
            meta.update(adapted.get("entities", {}))
            if adapted.get("exemplars"):
                 meta["exemplars"] = adapted["exemplars"] + meta.get("exemplars", [])
            if adapted.get("feature_names"):
                meta["feature_names"] = adapted["feature_names"]
        ...
        err_raw = float(best_err_t.item())

        # --- LEAP IV: TRACE SEAL + DIAGONAL CUTTER ---
        # Compute vector residual: r_t = frame - best_pred
        resid_vec = (frame - best_pred).detach() # (features,)

        # Compute TraceSeal score (non-negative energy in null space)
        err_ts = trace_seal.score(resid_vec)

        # Mix: best_err = mix * err_ts + (1-mix) * err_raw
        ts_mix = float(EIDOS_BRAIN_CONFIG.get("trace_seal_mix", 0.0))
        if not trace_seal.enabled:
            ts_mix = 0.0
        best_err = ts_mix * err_ts + (1.0 - ts_mix) * err_raw

        # "Consensus" is the best normal estimate; we feed this on non-surprise frames.
        consensus = best_pred.detach()

        # --- Error EMA + robust sigma (MAD) with optional Taper ---
        ema_count += 1
        if ema_count == 1:
            ema_err = best_err
        else:
        ...
        taper_mode = EIDOS_BRAIN_CONFIG.get("trace_seal_sigma_taper", "none")
        if taper_mode == "hann" and len(residual_history) > 10:
             # Weighted Std (Hann Window)
             hist_np = np.array(residual_history)
             N_h = len(hist_np)
             w = np.hanning(N_h)
             w_sum = w.sum()
             mean_w = (hist_np * w).sum() / w_sum
             var_w = (w * (hist_np - mean_w)**2).sum() / w_sum
             sigma = math.sqrt(var_w)
        else:
            # Standard MAD/Std Logic
        ...
        # --- TRACE SEAL UPDATE ---
        # Update covariance?
        # Default: update only if NOT surprise (keep baseline clean)
        update_on_surprise = bool(EIDOS_BRAIN_CONFIG.get("trace_seal_update_on_surprise", False))
        if update_on_surprise or (not is_surprise):
            trace_seal.update(resid_vec)
        
        # Log TraceSeal Diagnostics occasionally
        ts_diag_every = int(EIDOS_BRAIN_CONFIG.get("trace_seal_diag_every", 2000))
        if i > 0 and i % ts_diag_every == 0 and trace_seal.enabled:
            diag = trace_seal.diagnostics()
            te = diag.get("top_eigs", [])
            te_str = ",".join([f"{e:.2e}" for e in te[:3]])
            print(f"[TRACESEAL] step={i} updates={diag['updates']} expl={diag['explained']:.4f} logCond={diag['log_cond']:.2f} topEigs=[{te_str}]")

        # --- DEMO DASHBOARD (Lightweight) ---
        if EIDOS_BRAIN_CONFIG.get("demo_enable", False):
            demo_every = int(EIDOS_BRAIN_CONFIG.get("demo_every", 25))
            if i % demo_every == 0:
                # 1. Console Line
                is_surp_int = 1 if is_surprise else 0
                ts_exp = trace_seal.explained_ratio if trace_seal.enabled else 0.0
                print(f"[DEMO] t={i:06d} raw={err_raw:.4f} ts={err_ts:.4f} mix={best_err:.4f} z={z_score:.2f}/th={eff_z_thresh:.2f} expl={ts_exp:.3f} SURP={is_surp_int}")

                # 2. CSV Write
                if EIDOS_BRAIN_CONFIG.get("demo_write_csv", False):
                    csv_name = EIDOS_BRAIN_CONFIG.get("demo_out_csv", "demo_trace_seal.csv")
                    csv_path = os.path.join(EIDOS_DATA_ROOT, csv_name)
                    # Header check? minimal logic: just append
                    line_csv = f"{i},{err_raw},{err_ts},{best_err},{z_score},{eff_z_thresh},{is_surp_int},{ts_exp}\n"
                    if i == 0:
                        header = "t,err_raw,err_ts,best_err,z,z_th,surprise,explained\n"
                        hive_store.put(csv_path, header + line_csv, "text/csv")
                    else:
                        hive_store.append_line(csv_path, line_csv)

                # 3. Plot (Matplotlib) - strictly optional/non-blocking
                if EIDOS_BRAIN_CONFIG.get("demo_plot_matplotlib", False):
                    try:
                        import matplotlib.pyplot as plt
                        # Minimal "live" plot logic (very basic)
                        # We rely on user environment to handle windowing/backend
                        # This is usually for local demos, not headless Colab
                        pass 
                    except ImportError:
                        pass
        ...
        # Establish status for this step (Essential for Incident Cards)
        if i < WARMUP:
            status = "CALIBRATING"
        elif red_cooldown > 0:
             status = "RED"
        elif fatigue > EIDOS_BRAIN_CONFIG.get("plasticity_rest_threshold", 0.7):
             status = "AMBER"
        elif is_surprise:
             if z_score > 10.0:
                 status = "RED"
        ...
        # --- LEAP IV/V: INCIDENT / FORECAST LOGIC ---
        # 1. Top Drivers
        top_drivers = []
        if incident_emitter:
            # Simple attribution: absolute residual
            # resid_vec is (features,)
            # We want top K indices
            r_abs = torch.abs(resid_vec).cpu().numpy()
            top_idx = np.argsort(r_abs)[::-1][:5]
            fnames = meta.get("feature_names")
            for idx in top_idx:
                val = float(r_abs[idx])
                nm = fnames[idx] if fnames and idx < len(fnames) else f"feat_{idx}"
                top_drivers.append({"idx": int(idx), "name": nm, "value": val})

        # 2. Episode Indexing
        # Signature: We use h_x (content) as the primary signature for similarity
        # or we could use the bind(h_r, h_x). Let's use h_x if available.
        sig_vec = h_x if h_x is not None else np.zeros(EIDOS_BRAIN_CONFIG["hippocampus_dim"])
        
        if episode_index and i % 5 == 0: # Downsample slightly for perf? or every step.
             # Plan says "every step or at least when z_score is computed"
             episode_index.add(eidos_incident_cards.EpisodeRecord(
                 step=i,
                 ts=time.time(),
                 regime=status,
                 z=z_score,
        ...
        # 3. Forecast Engine Update
        if forecast_engine:
             forecast_engine.update(
        ...
        # 4. Incident Emission
        try:
            if incident_emitter and incident_emitter.should_emit(status, i):
                # Gather context
                sim_episodes = episode_index.topk(sig_vec, status, k=3)
                fc_risk = forecast_engine.risk(domain_name, status) if forecast_engine else {}
                
                # Procedural Recs
                actions = procedural_memory.rank_actions(sig_vec, status) if procedural_memory else []
                
                # Create Card
                card = eidos_incident_cards.IncidentCard(
                    incident_id=f"inc_{i}_{int(time.time())}",
                    domain=domain_name,
                    regime=status,
                    severity=status, # Redundant?
                    step=i,
                    ts=time.time(),
                    summary=f"Automated incident report for {status} anomaly at step {i}",
                    hypotheses=[{"label": "anomaly", "confidence": 1.0}], # Placeholder diagnoser
                    evidence={
                        "drivers": top_drivers,
                        "entities": meta.get("entities", {}),
                        "exemplars": meta.get("exemplars", []),
                        "baseline": {"z": z_score, "ema_err": ema_err, "sigma": sigma}
                    },
                    similar_episodes=sim_episodes,
                    invariant={}, # Todo: compute intersection
                    forecast=fc_risk,
                    actions=actions,
                    confidence=0.85
                )
                ...
                # Emit (Write to file)
                # We append to incident_cards.jsonl in artifact root
                card_path = os.path.join(ARTIFACT_ROOT_PREFERRED, "incident_cards.jsonl")
                try:
                    # Naive serialization
                ...
                incident_emitter.emit(card)
        except Exception as crash_e:
            print(f"!!! CRASH IN INCIDENT LOGIC: {crash_e}")
            import traceback
            traceback.print_exc()
            
        # 5. Forecast Periodic Emit
        if forecast_engine and EIDOS_BRAIN_CONFIG.get("forecast_mode") == "risk":
             emit_every = EIDOS_BRAIN_CONFIG.get("forecast_emit_every", 10)
             if i % emit_every == 0:
                 risk_data = forecast_engine.risk(domain_name, status)
                 fc_out = {
                     "step": i,
                     "ts": time.time(),
                     "domain": domain_name,
                 ...
                 fc_path = os.path.join(ARTIFACT_ROOT_PREFERRED, "forecast.jsonl")
                 try:
                     with open(fc_path, "a") as f:
                         f.write(json.dumps(fc_out, default=str) + "\n")
                 except: pass

        # 6. Procedural Reward Logic (Simple)
        if procedural_memory and status in ["AMBER", "RED"]:
            # naive: if we have active recommended actions, we typically wait to see if they worked.
            # For this v1, we just update reuse counts or mock rewards.
            pass
        ...
        # Inject TraceSeal into last_metrics for thermodynamics or other uses
        sentinel.last_metrics["trace_seal_err"] = float(err_ts) if trace_seal.enabled else 0.0
        ...
        recorder.record_step(
            ...
            trace_seal_err=err_ts if trace_seal.enabled else None,
            trace_seal_expl=trace_seal.explained_ratio if trace_seal.enabled else None,
            trace_seal_logcond=trace_seal.log_cond if trace_seal.enabled else None,
        )
