...
def build_run_lock(
    *,
    ...
    policy_config = asdict(PolicyConfig())
    representation_config = {
        "lift_ids": ["raw", "spectral", "multiscale", "geometry", "memory", "consensus"],
        "spectral_window": 64,
        "multiscale_alphas": [0.5, 0.1, 0.02],
        "min_calibration": 16,
        "p_min": 0.001,
        "past_history_limit": 2048,
        "nuisance_projection": "C1 raw lift only; frozen scenario projector",
    ...
    lock = {
        ...
        "policy_config_hash": "sha256:" + canonical_sha256(policy_config),
        "representation_config": representation_config,
        "representation_config_hash": "sha256:" + canonical_sha256(representation_config),
        "calibration_seeds": list(range(10, 20)),
        "heldout_seeds": list(range(100, 120)),
        "smoke_seeds": [0, 1],
        "reservoir": int(reservoir),
        "thresholds": dict(thresholds),
        "alert_budget": {"primary_fp_per_10k_nominal_frames": 5.0, "selection_split": "calibration_only"},
        "byte_operating_points": list(BYTE_OPERATING_POINTS),
        "byte_accounting": {
            "raw_dtype_bytes": 8,
            "payload_bytes_per_feature": {
                "reference_or_null": 1,
                "quantized_residual": 2,
                "structured_residual": 4,
                "raw_frame_plus_full_context": 8,
            },
            "included_components": ["payload", "index", "cards", "model_state", "manifest"],
        },
        "metric_config": {
            "paired_bootstrap_unit": "seed_or_source_window",
            ...
            "scalar_sensitivity_grid": registered_sensitivity_grid(),
        ...
        "commands": {
            "tests": f"python -m pytest -c eidos/pytest.ini eidos/tests eidos/repo/tests --junitxml={artifact_root_text}/provenance/pytest_results.xml",
            "smoke": f"python eidos/tools/run_grand_proof_v1.py run --stage smoke --seeds 0,1 --reservoir 128 --out {artifact_root_text}",
            "calibration": f"python eidos/tools/run_grand_proof_v1.py run --stage calibration --seeds 10-19 --reservoir {int(reservoir)} --out {artifact_root_text}",
            "heldout": f"python eidos/tools/run_grand_proof_v1.py run --stage heldout --seeds 100-119 --reservoir {int(reservoir)} --out {artifact_root_text}",
        },
