...
test("operator and full-capacity gates stay closed before any dispatch", async () => {
  ...
  try {
    ...
    const testKey = "approved-test-key";
    delete process.env.EIDOS_OPERATOR_TOKEN;
    process.env.EIDOS_TEST_ACCESS_GRANTS = JSON.stringify([{ id: "pilot-01", sha256: createHash("sha256").update(testKey).digest("hex"), expiresAt: new Date(Date.now() + 60_000).toISOString() }]);
    assert.equal(isOperatorAuthConfigured(), true);
    authorizeOperator(new Request("http://test", { headers: { Authorization: `Bearer ${testKey}` } }));
    assert.throws(() => authorizeOperator(new Request("http://test", { headers: { Authorization: "Bearer unapproved" } })), /OPERATOR_AUTH_REQUIRED/);
    process.env.EIDOS_TEST_ACCESS_GRANTS = JSON.stringify([{ id: "expired", sha256: createHash("sha256").update(testKey).digest("hex"), expiresAt: "2020-01-01T00:00:00.000Z" }]);
    assert.equal(isOperatorAuthConfigured(), false);
    assert.throws(() => authorizeOperator(new Request("http://test", { headers: { Authorization: `Bearer ${testKey}` } })), /OPERATOR_AUTH_NOT_CONFIGURED/);
    process.env.EIDOS_OPERATOR_TOKEN = "local-test-only";
    delete process.env.EIDOS_TEST_ACCESS_GRANTS;
