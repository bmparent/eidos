...
const text = (value: string) =>
  ...
  ).replace(/(^|[^a-z0-9_@])@([a-z][a-z0-9_]{2,23})(?![a-z0-9_])/gi, (_match,before,name) => `${before}<a href="${name.toLowerCase()==='eidos'?'/community':'/members/'+name.toLowerCase()}">@${name}</a>`);
