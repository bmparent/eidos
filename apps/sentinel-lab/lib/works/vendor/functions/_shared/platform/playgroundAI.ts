...
import {aiContext,aiTarget,applyProposal,operationSchema,validateProposal,type Proposal} from '../../../src/playground/aiOperations';
...
export async function requestAI(env:PlatformEnv,owner:string,input:Record<string,unknown>) {
 ...
 const project=structuredClone(known.document);
 let target:ReturnType<typeof aiTarget>;try{target=aiTarget(project,context.section.id);}catch{throw new HttpError(409,'Save the new section before asking AI to edit it.');}
 const section=target.section,selected=target.node?.id||section.id;
 ...
 if(target.node)target.node.text=context.section.title;
 else for(const k of ['title','description','cta','href','layout','style'] as const)if(k in context.section)Object.assign(section,{[k]:context.section[k]});
 ...
 const scope=input.scope as 'section'|'page',bounded=aiContext(valid,selected,scope),content=JSON.stringify({context:bounded,request:input.request});
 ...
 try {if(provider.status!=='completed')throw Error('No complete output');if(kind==='text'){proposal=validateProposal(provider.proposal);applyProposal(valid,selected,scope,proposal);}else{if(!provider.image||!env.EIDOS_VALIDATE_PLAYGROUND_IMAGE)throw Error('Missing image validator');await env.EIDOS_VALIDATE_PLAYGROUND_IMAGE(provider.image);image=provider.image;}}catch {state='rejected';}
 const result=state==='completed'?{proposal,image,documentId:input.documentId,baseHash:input.baseHash,sectionId:selected,scope,model,providerId:provider.requestId,quality:kind==='image'?'low':undefined}:null;
