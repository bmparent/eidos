...
CREATE TABLE IF NOT EXISTS eidos_members (
...
CREATE TABLE IF NOT EXISTS eidos_member_sessions (
 token_hash TEXT PRIMARY KEY, member_id TEXT NOT NULL REFERENCES eidos_members(id), expires INTEGER NOT NULL
);
...
CREATE TABLE IF NOT EXISTS eidos_member_keys (
 id TEXT PRIMARY KEY, member_id TEXT NOT NULL REFERENCES eidos_members(id), key_hash TEXT NOT NULL UNIQUE,
...
CREATE TABLE IF NOT EXISTS eidos_bookmarks (
 member_id TEXT NOT NULL REFERENCES eidos_members(id), slug TEXT NOT NULL, created_at TEXT NOT NULL,
 PRIMARY KEY(member_id,slug)
);
CREATE TABLE IF NOT EXISTS eidos_mentions (
 id TEXT PRIMARY KEY, member_id TEXT NOT NULL REFERENCES eidos_members(id), source_id TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS eidos_mail_deliveries (
 id TEXT PRIMARY KEY, member_id TEXT NOT NULL REFERENCES eidos_members(id), through_date TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS eidos_unsubscribe_tokens (
 token_hash TEXT PRIMARY KEY, member_id TEXT NOT NULL REFERENCES eidos_members(id)
);
