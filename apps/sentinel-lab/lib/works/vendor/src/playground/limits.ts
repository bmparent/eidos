...
export function cloudPreflight(document: unknown) {
  ...
  const sections = document && typeof document === 'object' && 'sections' in document && Array.isArray(document.sections) ? document.sections as {image?:string}[] : [];
  const images=sections.map(s=>typeof s.image==='string'?s.image:'');
