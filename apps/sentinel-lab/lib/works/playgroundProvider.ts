...
export function playgroundProvider(key:string|undefined):AIProvider|undefined {
 ...
 return async input=>{
  ...
  {model:input.model,store:false,input:[{role:'developer',content:[{type:'input_text',text:input.instructions}]},{role:'user',content:input.content}],text:{format:{type:'json_schema',name:'playground_edit',strict:true,schema:input.schema}},max_output_tokens:input.maxOutputTokens,prompt_cache_key:input.cacheKey};
