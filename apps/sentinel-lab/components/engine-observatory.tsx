  ...
  return <section ref={shell} className="observatory" aria-labelledby={`${id}-title`}>
    ...
    <div className="reservoir-stage">
      <svg viewBox="0 0 680 380" role="img" aria-label="Rotatable three-dimensional projection of sampled reservoir states. Use the frame and viewing-angle sliders to explore." onPointerDown={(event) => { drag.current = { x: event.clientX, yaw }; event.currentTarget.setPointerCapture(event.pointerId); setMotion(false); }} onPointerMove={(event) => { if (drag.current) setYaw(drag.current.yaw + (event.clientX - drag.current.x) / 180); }} onPointerUp={() => { drag.current = null; }} onPointerCancel={() => { drag.current = null; }}>
        ...
        {projected.map((p, i) => <circle key={i} cx={p.x} cy={p.y} r={i === position ? 3.8 : Math.max(0.7, 1.7 - p.depth / 300)} fill={i <= position ? "#b5aaff" : "#527c89"} opacity={i <= position ? 0.85 : 0.35} />)}
    ...
    <div className="observatory-footnote"><p>Three principal components preserve {((data.geometry?.variance_explained ?? 0) * 100).toFixed(1)}% of sampled state variance. Color follows sample order. Readouts use recorded frame {String(row.step ?? "—")}; channel scales differ. This projection supports inspection, not proof of detection quality.</p><button onClick={() => downloadJson(isReference ? reference : data, isReference ? "eidos-recorded-reference.json" : "eidos-engine-diagnostics.json")}>Inspect the source data ↗</button></div>
  </section>;
}
