    ...
    <section className="real-data-page" aria-labelledby="real-data-title">
      ...
      <div className="real-workspace">
        ...
        <aside className="run-lock-panel">
          ...
          <label className="operator-token"><span>Operator or test access key</span><input type="password" autoComplete="off" value={operatorToken} onChange={(event) => setOperatorToken(event.target.value)} placeholder="Enter a key to launch or retrieve a run" /></label>
          <p className="token-note">Kept in memory for this page only. Readiness checks your settings. Compute startup and dataset access are verified after launch. <a href="https://eidos-works.com/lab#request-access" target="_blank" rel="noreferrer">Request reviewed test access</a>.</p>
