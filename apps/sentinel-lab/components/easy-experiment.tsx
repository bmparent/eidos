...
export function EasyExperiment() {
  ...
  async function restoreJob(saved: LabDocument) {
    const generation = epoch.current;
    if (saved.status === "completed") {
      if (saved.result?.runId) await openRun(saved.result.runId);
      else if (saved.result?.datasetId) loadDataset(await api(`datasets/${encodeURIComponent(saved.result.datasetId)}`));
      else throw new Error("The completed job has no saved dataset or run reference. Inspect it in the detailed workspace.");
    }
    if (generation === epoch.current) { setJob(saved); setPaused(false); }
  }
