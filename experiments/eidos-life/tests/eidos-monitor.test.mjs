...
test('red regime triggers collapse protection', ()=>{
  ...
  assert.equal(out.metrics.rawRegime,'RED'); assert.equal(out.metrics.regime,'AMBER'); assert.equal(out.metrics.redFlicker,1); assert.equal(out.rulePreset.reseed,true);
});
